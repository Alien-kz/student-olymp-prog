	...
	with open(template_file, 'r') as temp_file:
		...
		for temp_info in list_of_temp_info:
			...
			if (tex_status):
				...
				pictures = glob.glob(input_directory + "pictures/" + temp_info['file'] + "*.tex")
				for pic_full_path in pictures:
					pic_file_name = pic_full_path.split('/')[-1]
					print("    picture:", pic_file_name)
					output_directory_picture = output_directory + "pictures/"
					if not os.path.exists(output_directory_picture):
						os.system("mkdir " + output_directory_picture)
					os.system("cp " + pic_full_path + " " + output_directory_picture)
